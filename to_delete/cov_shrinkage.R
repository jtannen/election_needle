...
get_svd <- function(
  ...
  fitted <- row_score_mat %*% diag(e$values[1:n_svd]) %*% t(col_score_mat)
...
get_shrinkage_cov <- function(wide_mat_demeaned, row_means, col_means, n_svd, ...){
...
get_svd_cov <- function(wide_mat_demeaned, row_means, col_means, n_svd, get_group_from_rowname, ...){
  ...
  for(group in unique(group_resid$group)){
    rows <- which(
      get_group_from_rowname(row.names(wide_mat)) == group
    )
